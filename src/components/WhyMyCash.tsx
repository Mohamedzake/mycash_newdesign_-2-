...
const WhyMyCash = async () => {
  const t = await getTranslations("whymycash");
  const solutionsData = [
    {
      id: 1,
    ...
    <section className="text-center pt-12 m-auto container ">
      ...
      <div className="grid lg:grid-cols-6 py-12">
        {solutionsData.map((item) => (
          <div
            key={item.id}
            className="w-[90%] m-auto rounded-t-lg shadow-lg bg-re col-span-2 mb-10"
          >
            <h3 className="text-xl font-bold bg-primary-blue text-white py-4 rounded-t-full">
              {item.title}
            </h3>
            <p className="text-gray-600 rounded-b-xl text-xl px-6 py-8 text-balance leading-8">
              {item.desc}
            </p>
          </div>
        ))}
